...
const char * const BITCOIN_CONF_FILENAME = "mewcoin.conf";
const char * const BITCOIN_PID_FILENAME = "mewcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "mewcoin" is a composite category enabling all Mewcoin-related debug output
            if(ptrCategory->count(string("mewcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MewcoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MewcoinCore
    // Mac: ~/Library/Application Support/MewcoinCore
    // Unix: ~/.mewcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MewcoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MewcoinCore";
#else
    // Unix
    return pathRet / ".mewcoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty mewcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override mewcoin.conf
